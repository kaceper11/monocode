...
import { linkedWorkItemNeedsAccount, sessionWorkItems } from "../../sessions/model/sessionWorkItem";
...
export function linkedWorkItemUpdateKey(
  ...
  // Fork-era sessions stored Azure links as "azure"; upstream's provider is "azuredevops".
  const provider = (item.provider as string) === "azure" ? "azuredevops" : item.provider ?? "github";
  const identity =
    provider === "github"
...
export function linkedWorkItemTargets(
  ...
  for (const session of sessions) {
    ...
    for (const linked of sessionWorkItems(session)) {
      ...
      if (linked.provider && !["github", "jira", "azure", "azuredevops"].includes(linked.provider)) continue;
