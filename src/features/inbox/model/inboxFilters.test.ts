...
describe("visibleInboxSources", () => {
  it("drops sources that are known to be disconnected", () => {
    ...
    ).toEqual(["github", "linear", "gitlab", "jira", "azuredevops"]);
  });
...
describe("connectableInboxSources", () => {
  it("offers only the sources confirmed to be disconnected", () => {
    ...
    ).toEqual(["linear", "jira", "azuredevops"]);
    expect(
      connectableInboxSources({
        github: false,
        linear: false,
...
it("keeps Jira and Azure DevOps connection choices independent", () => {
  const connections = {
    github: true,
    linear: false,
    gitlab: false,
  ...
  expect(resolveInboxSource("azuredevops", connections)).toBe("github");
  expect(visibleInboxSources(connections)).toEqual(["github", "jira"]);
  expect(connectableInboxSources(connections)).toEqual([
    "linear",
    "gitlab",
    "azuredevops",
  ]);
...
describe("selected-provider filter isolation", () => {
  ...
  it("does not let a removed source-visibility preference redirect the saved tab", () => {
    ...
    localStorage.setItem("monocode.inboxSource", "azuredevops");
    expect(loadInboxSource()).toBe("azuredevops");
  });

  it("counts only filters usable by the selected provider", () => {
    const unrelated = {
      ...DEFAULT_INBOX_FILTERS,
      assignedToMe: true,
      hiddenProjects: ["/tmp/web"],
    ...
    expect(hasActiveInboxFilters(unrelated, "azuredevops")).toBe(true);
    const draft = {
      ...DEFAULT_INBOX_FILTERS,
      status: { ...DEFAULT_INBOX_FILTERS.status, draft: true },
    };
    expect(hasActiveInboxFilters(draft, "jira")).toBe(false);
    expect(hasActiveInboxFilters(draft, "azuredevops")).toBe(true);
    expect(hasActiveInboxFilters({ ...draft, time: "today" }, "jira")).toBe(
      true,
    );
  });
});
