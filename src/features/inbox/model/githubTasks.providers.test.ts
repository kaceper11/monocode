...
describe.each([
  ...
  it.each([false, true])(
    ...
    async (assignedToMe) => {
      ...
      await expect(
        ...
        items: [],
        errors: { [provider]: "issue unavailable", jira: "Connect Jira Cloud in Settings to see assigned issues." },
      });
    },
  );
  ...
  it("fetches assigned items even without local projects", async () => {
    ...
    expect(result.errors).toEqual({ jira: "Connect Jira Cloud in Settings to see assigned issues." });
    expect(list).toHaveBeenCalledTimes(2);
  });
