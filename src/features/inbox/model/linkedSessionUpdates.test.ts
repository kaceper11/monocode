...
it("ignores unbound legacy jira targets and any old unscoped activity cache", () => {
  const legacy = { ...linked, provider: "jira" as const, account: undefined };
  const saved = session("legacy", 100, { linkedWorkItem: legacy });
  const snapshots = new Map([[linkedWorkItemUpdateKey(legacy), remote(200, { provider: "jira", account: "current-account" })]]);
  expect(linkedWorkItemTargets([saved])).toEqual([]);
  expect(linkedSessionUpdates([saved], snapshots).size).toBe(0);
});

it("matches fork-era azure links to azuredevops inbox items without an account", () => {
  const url = "https://dev.azure.com/org/Product/_git/repo/pullrequest/42";
  const legacy = { ...linked, provider: "azure" as unknown as "azuredevops", account: undefined, url };
  const saved = session("legacy", 100, { linkedWorkItem: legacy });
  const item = remote(200, { provider: "azuredevops", url });
  const snapshots = new Map([[linkedWorkItemUpdateKey(item), item]]);
  expect(linkedWorkItemTargets([saved])).toEqual([
    { key: linkedWorkItemUpdateKey(item), item: legacy },
  ]);
  expect(linkedSessionUpdates([saved], snapshots).has("legacy")).toBe(true);
});
