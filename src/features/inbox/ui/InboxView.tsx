...
import {
  ...
  type InboxThread,
  type InboxDiff,
} from "../../sessions/model/inboxProvider";
import { refreshLinkedWorkItem } from "../../sessions/model/linkedWorkItemRefresh";
import { linkedWorkItemNeedsAccount } from "../../sessions/model/sessionWorkItem";
import { LinkedWorkItemAccount } from "../../sessions/ui/LinkedWorkItemAccount";
  ...
  useEffect(() => {
    ...
    const pending =
      !target.provider || target.provider === "github"
        ? githubWorkItem(cwd, target.repo, target.kind, target.number).then(
            (item) => ({
              ...item,
              projectPath: cwd,
...
export function LinkedWorkItemPanel({
  ...
  const cachedItem = github
    ? peekGithubWorkItem(target.repo, target.kind, target.number)
    : null;
  ...
  useEffect(() => {
    ...
    const pending = github
      ? githubWorkItem(cwd, target.repo, target.kind, target.number).then(
          (item) => ({ ...item, provider: "github" as const }),
        )
      : refreshLinkedWorkItem(cwd, target);
    void pending
      .then((next) => {
        ...
        if (!next)
          throw new Error(
            "Could not load the linked item. Check its provider connection.",
          );
      ...
      <div className="min-h-0 min-w-0 flex-1">
        {needsAccount ? (
          <div className="flex h-full items-center justify-center px-8 text-center">
            <LinkedWorkItemAccount
              key={`${target.provider}:${target.url}`}
              target={target}
              cwd={cwd}
              onBind={onBindAccount ?? (() => false)}
            />
          </div>
        ) : item ? (
          <InboxDetail
          ...
          <div className="flex h-full flex-col items-center justify-center gap-3 px-8 text-center">
            ...
            >
              ...
              {github ? "Open on" : "Open in"}{" "}
              {INBOX_SOURCE_LABELS[target.provider ?? "github"]}
            </button>
          </div>
        ) : (
...
export function InboxDetail({
  ...
  const externalActionLabel = `${
    isPr
      ...
      : item.provider === "linear" || item.provider === "jira"
        ? "Open in"
        : "Open on"
  } ${INBOX_SOURCE_LABELS[item.provider]}`;
  const upstreamProvider =
    item.provider === "github" || gitlab || linear || azuredevops;
  ...
  const [thread, setThread] = useState<InboxThread | null>(cachedThread);
  ...
  const chooseStartProject =
    linear || ((gitlab || azuredevops || item.provider === "jira") && !item.projectPath);
  ...
  useEffect(() => {
    ...
    void adapter
      ...
      .catch((error: unknown) => {
        if (!cancelled && !(cached && upstreamProvider))
  ...
  const postComment = async (body: string) => {
    ...
    try {
      ...
      try {
        setThread(await adapter.thread(true));
      } catch (error) {
            ...
            {isPr && tab === "code" ? (
              ...
              <>
                ...
                {details?.attachments?.length ? (
                  <TicketImages item={item} attachments={details.attachments} />
                ) : null}
                <InboxComments
                  thread={thread}
