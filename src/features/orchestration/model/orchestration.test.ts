...
describe("local orchestration", () => {
  it.each([false, true])("uses the lead checkout host for approval, listing, delegation and worker revalidation (worktree=%s)", async worktree => {
    ...
    f.host.choices = vi.fn((host: string) => host === checkout
      ? [{ harness: "codex" as const, models: [{ id: "codex:test", name: "Linux" }] }]
      : []);
    await f.manager.startApproved("lead", "proposal-host", { ...proposal(), cwd, checkoutCwd: checkout });
    await f.call("list");
    await f.delegate(["other"]);
    await vi.waitFor(() => expect(f.host.createWorker).toHaveBeenCalled());
    ...
    await vi.waitFor(() => expect(f.store.resolvePath).toHaveBeenCalledWith(
      "/home/me/repo/src/file.ts", task.workspace?.checkoutCwd ?? checkout,
    ));
  });
