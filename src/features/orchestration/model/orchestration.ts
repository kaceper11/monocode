...
import { HARNESSES, sessionWorkCwd, type HarnessId, type Session } from "../../sessions/model/session";
...
export type OrchestrationHost = {
  ...
  sessions(): Session[];
  choices(cwd: string): { harness: HarnessId; models: { id: string; name: string }[] }[];
  createWorker(
...
type Storage = {
  ...
  resolvePath(path: string, cwd: string): Promise<string>;
};
const storage: Storage = {
  ...
  resolvePath: (path, cwd) => invoke("control_write_path", { path, cwd }),
};
...
export function workerTurnPrompt(
  ...
  const scratch = scratchDir
    ? ` Temporary helpers and test output may be written in your private scratch directory: ${JSON.stringify(wslLocation(scratchDir)?.path ?? scratchDir)}. TMPDIR, TMP and TEMP point there. Use this directory for scratch files; do not write elsewhere outside the project. Deliver final changes in your assigned project files.`
    : "";
...
export class Orchestrator {
  ...
  async startApproved(
    ...
    const available = this.host!.choices(sessionWorkCwd(lead));
  ...
  private async startRun(
    ...
    const available = this.host!.choices(sessionWorkCwd(lead)).map((choice) => choice.harness);
    if (
  ...
  private async execute(
    ...
    switch (action) {
      case "list":
        return {
          run: this.view(run),
          harnesses: this.host!.choices(orchestrationCheckoutCwd(run))
      ...
      case "delegate": {
        ...
        const choice = this.host!.choices(orchestrationCheckoutCwd(run)).find(
      ...
      for (const initial of this.runs) {
        ...
        for (const initialTask of initialRun.tasks) {
          ...
          try {
            if (
              !this.host
                .choices(orchestrationCheckoutCwd(run))
                .some(
                  (choice) =>
  ...
  observe(id: string, event: HarnessEvent) {
    ...
    const check = (async () => {
      for (const path of paths) {
        ...
        let resolved: string;
        try {
          resolved = await this.store.resolvePath(absolute, task.workspace?.checkoutCwd ?? orchestrationCheckoutCwd(run));
        } catch (error) {
          if (stillRunning())
