...
function Row({
  ...
  return (
    <div
      id={id ? settingDomId(id) : undefined}
      data-setting-id={id}
      className={`settings-row flex items-start border-b border-content/5 px-4 py-3.5 transition-colors last:border-b-0 ${
        stacked ? "flex-col gap-4" : "gap-6"
      } ${flash ? "bg-accent/10" : ""}`}
    >
      <div className="min-w-0 flex-1">
      ...
      <div
        className={
          stacked
            ? "w-full min-w-0"
            : "settings-row-control flex min-w-0 max-w-[60%] shrink-0 flex-wrap items-center justify-end gap-2"
        }
      >
        {children}
      </div>
