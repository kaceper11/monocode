...
import { shortError } from "./taskOps";
...
export function TaskGitActions({ targets, all = false, disabled = false, onDone, layer = LAYER.popover }: {
  targets: readonly Target[]; all?: boolean; disabled?: boolean; onDone?: () => void; layer?: number;
}) {
    ...
    try {
      for (const target of unique) {
        ...
        try {
          ...
          setResults(rows => [...rows, { name, message: action === "fetch" ? "Fetched" : "Pulled" }]);
        } catch (error) {
          setResults(rows => [...rows, { name, message: shortError(error), error: String(error) }]);
        }
      }
      onDone?.();
    } finally {
  ...
  return <div className="min-w-0 text-[11px]">
    ...
    <div aria-live="polite" className="space-y-1">{results.map((result, index) => <p key={index} role={result.error ? "alert" : "status"} className={`mt-1 [overflow-wrap:anywhere] ${result.error ? "text-red-600 dark:text-red-300" : "text-content/50"}`}>{all ? `${result.name}: ` : ""}{result.message}</p>)}</div>
  </div>;
}
