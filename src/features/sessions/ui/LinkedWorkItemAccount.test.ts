...
vi.mock("../model/jira", () => ({ jiraConnected: api.status, JIRA_CHANGE_EVENT: "jira-changed" }));
vi.mock("../model/linkedWorkItemRefresh", () => ({ refreshLinkedWorkItem: api.refresh }));
...
it("binds a legacy jira link only after the displayed account is chosen and checked", async () => {
  ...
  expect(api.refresh).toHaveBeenCalledWith("/repo", { ...target, account: "account-a", site: "https://team.example" });
