...
it("requires explicit account binding for legacy jira links in both matching paths", () => {
  const linked = { provider: "jira" as const, kind: "issue" as const, number: 7, repo: "", url: "https://example.test/items/7" };
...
it("matches legacy azure links to azuredevops items without account binding", () => {
  const linked = { provider: "azure" as unknown as "azuredevops", kind: "issue" as const, number: 7, repo: "", url: "https://dev.azure.com/org/Product/_workitems/edit/7" };
  const item = { ...linked, provider: "azuredevops" as const } as InboxItem;
  const session = { linkedWorkItem: linked };
  expect(inboxItemMatchesLinkedWorkItem(item, linked)).toBe(true);
  expect(relatedFromIndex(item, indexByWorkItem([session], sessionWorkItems))).toEqual([session]);
});
