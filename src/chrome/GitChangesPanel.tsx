...
import { gitPrProvider } from "../lib/gitPrProvider";
...
type PrDestination = Awaited<ReturnType<typeof gitPrProvider>>;
const githubPrByCwd = new Map<string, { key: string; destination: PrDestination }>();
...
function ChangedFiles({
  ...
  const { destination, error: prError, reload: reloadPr } = usePrStatus(cwd, index?.branch, index?.remote);
  const pr = destination?.pr ?? null;
  ...
  const canCommitPushPr = !!destination && canCommitPush && !hasOpenPr && !onDefault;
  ...
  const recordPrActivity = (number = pr?.number) => {
    if (!number || destination?.provider !== "github") return;
  ...
  const openCreatedPr = async () => {
    ...
    if (!destination) throw new Error("Choose a PR destination first");
    const url = await destination.create(content);
  ...
  return (
    ...
    >
      <div className="shrink-0 border-b border-stroke p-2">
        ...
        {index?.remote && (destination?.provider === "azure" || prError) ? (
          <p className="mt-1.5 break-words text-[11px] text-content/50" role={prError ? "status" : undefined}>
            {prError ?? destination?.label}
          </p>
        ) : null}
        {index ? (
          <GitSyncActions
...
function usePrStatus(cwd: string, branch: string | null | undefined, remote: string | null | undefined) {
  const key = JSON.stringify([cwd, branch, remote]);
  const [state, setState] = useState<{ key: string; destination?: PrDestination; error?: string }>(() => {
    const cached = githubPrByCwd.get(cwd);
    return { key, destination: cached?.key === key ? cached.destination : undefined };
  });
  const [nonce, setNonce] = useState(0);
  const reload = useCallback(() => setNonce((value) => value + 1), []);
  useEffect(() => {
    if (!cwd || cwd === "~" || !branch || !remote) return;
    let cancelled = false;
    let generation = 0;
    let github = githubPrByCwd.get(cwd)?.key === key;
    const load = () => {
      const current = ++generation;
      void gitPrProvider(cwd, remote, branch).then(
        (destination) => {
          if (cancelled || current !== generation) return;
          github = destination.provider === "github";
          if (github) githubPrByCwd.set(cwd, { key, destination });
          else githubPrByCwd.delete(cwd);
          setState({ key, destination });
        },
        (error) => {
          if (cancelled || current !== generation) return;
          githubPrByCwd.delete(cwd);
          setState({ key, error: String(error) });
        },
      );
    };
    const onAzureChange = () => {
    ...
    window.addEventListener(AZURE_CHANGE_EVENT, onAzureChange);
    return () => {
      cancelled = true;
      window.removeEventListener("focus", load);
      window.removeEventListener(AZURE_CHANGE_EVENT, onAzureChange);
    };
  }, [cwd, branch, remote, key, nonce]);
