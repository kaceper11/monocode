...
import * as adapters from "../lib/inboxProvider";
import * as github from "../lib/githubTasks";
...
vi.mock("./AgentMarkdown", () => ({ AgentMarkdown: ({ text }: { text: string }) => createElement("p", null, text) }));
it.each(["jira", "azure"] as const)("uses upstream Ask and Send controls with %s identity and the chosen project", async provider => {
  vi.stubGlobal("IS_REACT_ACT_ENVIRONMENT", true);
  const item: InboxItem = { provider, kind: provider, account: "Ada", site: provider === "jira" ? "https://team.atlassian.net" : "https://dev.azure.com/team", id: "42", number: 42, identifier: "ENG-42", title: "Ticket 42", url: "https://example.test/42", projectPath: "", repo: "", projectName: "Engineering", state: "In review", updatedAt: "", labels: [], assignees: [], draft: false };
  vi.mocked(invoke).mockImplementation(async (_cmd, args) => args?.comments || args?.discussion ? { comments: [], total: 51, more: true } : { fields: { description: "Shared description", creator: { displayName: "Ada" }, "System.Description": "Shared description", "System.CreatedBy": { displayName: "Ada" } } });
  ...
  try {
    ...
    expect(onStart).toHaveBeenCalledWith(expect.objectContaining({ provider, id: "42", site: item.site, account: "Ada", projectPath: "/local/project" }), "Shared description");
    expect(host.textContent).not.toContain("Send to task");
    expect(host.textContent).toContain(`Latest comments · more on ${provider === "jira" ? "Jira" : "Azure DevOps"}`);
