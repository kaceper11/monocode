...
it.each(["jira", "azure"])("loads authenticated %s images separately from agent context and retries failures", async (provider) => {
  ...
  const item = { provider, site, account: "account-a", id: "42", url: provider === "azure" ? `${site}/Product/_workitems/edit/42` : `${site}/browse/ENG-42` } as InboxItem;
  ...
  try {
    ...
    expect(invoke).toHaveBeenCalledExactlyOnceWith(`${provider}_image`, { site: item.site, accountId: "account-a", id: "42", attachmentId: "12" });
