...
describe("Hermes live ACP sequence", () => {
  ...
  it.each(["allow_once", "opaque-allow"])("uses Hermes permission option ids for supervised approvals (%s)", async (allowId) => {
    const events: HarnessEvent[] = [];
    const turn = sendHermesTurn({
      sessionId: `hermes-live-permission-${allowId}`,
      cwd: "/repo",
      model: "hermes:nous:hermes-4",
      runtimeMode: "supervised",
      text: "run git",
    ...
    respondHermesApproval(`hermes-live-permission-${allowId}`, events.find((e) => e.type === "approval.requested")!.requestId, "allow");
    await waitFor(
    ...
    await turn;
    await stopHermesSession(`hermes-live-permission-${allowId}`);
  });
});
