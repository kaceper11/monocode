...
async function handlePermission(
  ...
  if (live.cancelled || live.muteUpdates) {
    await respondPermission(live, id, undefined);
    return;
  }
  if (request.callId) {
    live.onEvent({
  ...
  if (live.planning) {
    ...
    await respondPermission(
      live,
      id,
      permissionOptionId(readOnly ? "allow" : "deny", request.optionIds, request.options),
    );
    return;
  }
  ...
  await respondPermission(
    live,
    id,
    permissionOptionId(decision, request.optionIds, request.options),
  );
}
