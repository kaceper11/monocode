...
import { connectWslProject } from "../../features/sessions/model/wsl";
vi.mock("@tauri-apps/api/core", () => ({ invoke: vi.fn().mockResolvedValue(false) }));
vi.mock("@tauri-apps/api/event", () => ({ listen: vi.fn().mockResolvedValue(() => {}) }));
vi.mock("../../features/sessions/model/wsl", () => ({ connectWslProject: vi.fn(), invalidateWslDiscovery: vi.fn(), wslDistributions: vi.fn().mockResolvedValue(["Ubuntu"]) }));
