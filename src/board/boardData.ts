...
import type { GitPr, GitPrCheck } from "../lib/fs";
import {
  sessionDisplayTitle,
  sessionNeedsInput,
  type LinkedWorkItem,
 ...
 * a session doing unlinked work, or a board-local note. Sessions and Azure
 * delivery runs attach to item cards instead of spawning their own.
 */
...
export type BoardCard = {
  ...
  hasUpdate: boolean;
  /** Azure delivery runs matched to a joined session's branch. */
  ciTotal: number;
  ciFailing: number;
  ciRunning: number;
  updatedAt: number;
...
export function ticketKeys(link: LinkedWorkItem): string[] {
  ...
  if (link.provider === "azure" && link.number > 0)
 ...
 * or the branch Azure reports for its delivery. Matched on non-alphanumeric
 * boundaries so `PROJ-123` doesn't join `PROJ-1234` or `APROJ-123`. */
export function itemMatchesTicketKey(item: InboxItem, key: string): boolean {
  const haystack = [
    item.identifier,
    item.title,
    item.url,
    item.delivery?.branch,
    item.delivery?.targetBranch,
  ]
    .filter(Boolean)
    .join("\n");
  return new RegExp(
...
export function boardLinkFromInboxItem(
  ...
  if (item.kind === "ci" || !item.url) return null;
  const linked = linkedWorkItemFromInboxItem(item);
...
function ciRunState(item: InboxItem): "failing" | "running" | "other" {
  const state = (item.state ?? "").toLowerCase();
  if (/fail|error|reject/.test(state)) return "failing";
  // Cancelled is terminal — not running, and not worth a red badge.
  if (/progress|running|queued|pending|notstarted/.test(state))
    return "running";
  return "other";
}
...
export function buildBoardCards(input: BoardInput): BoardCard[] {
  ...
  const ciByBranch = new Map<string, InboxItem[]>();
  ...
  const taskBranches = new Map<string, MutableCard>();
  ...
  for (const task of input.tasks ?? []) {
    ...
    for (const ws of task.workstreams) {
      ...
      taskBranches.set(shortRef(ws.branch), card);
      card.branches.add(shortRef(ws.branch));
  ...
  const taskForItem = (
    ...
    // A PR joins a task when it runs on a task branch or carries a ticket key.
    const branch = item.delivery?.branch
      ? shortRef(item.delivery.branch)
      : undefined;
    if (branch && taskBranches.has(branch))
      return { card: taskBranches.get(branch)!, via: "branch" };
  ...
  for (const item of input.items) {
    // Delivery runs are status, not work — they badge the card whose
    // branch they ran on rather than taking a column slot of their own.
    if (item.kind === "ci") {
      const branch = item.delivery?.branch
        ? shortRef(item.delivery.branch)
        : undefined;
      if (branch) {
        const list = ciByBranch.get(branch) ?? [];
        list.push(item);
        ciByBranch.set(branch, list);
      }
      continue;
    }
  ...
  const attachToCard = (
    ...
    if (task) {
      ...
      if (branch) task.branches.add(shortRef(branch));
      return task;
    }
    ...
    if (branch) card.branches.add(shortRef(branch));
    return card;
  };
  ...
  for (const session of input.sessions) {
    ...
    if (bound) {
      ...
      if (session.branch) bound.card.branches.add(shortRef(session.branch));
      continue;
    }
    ...
    if (!links.length) {
      const card: MutableCard = {
        ...
        branches: new Set(session.branch ? [shortRef(session.branch)] : []),
        workstreamRows: new Map(),
      };
      cards.set(card.id, card);
      continue;
    ...
    for (const linked of links) {
      attachToCard(linked, ref, session.branch ?? undefined, session.cwd);
    }
  }
  ...
  for (const summary of input.summaries) {
    ...
    if (bound) {
      pushSession(bound.card, ref);
      if (summary.branch) bound.card.branches.add(shortRef(summary.branch));
      continue;
    }
    for (const linked of sessionWorkItems(summary)) {
      attachToCard(linked, ref, summary.branch ?? undefined, summary.cwd);
    }
  }
  ...
  for (const card of cards.values()) {
    ...
    for (const branch of card.branches) {
      for (const run of ciByBranch.get(branch) ?? []) {
        card.ciTotal += 1;
