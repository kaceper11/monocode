...
/** Probe one workstream's PR + checks through the remote's provider —
 * `gitPrProvider` routes GitHub via `gh` and Azure Repos via the API. */
export async function probeWorkstream(
  ...
  try {
    ...
    const remote =
      context.remotes.find((entry) => entry.name === "origin") ??
      context.remotes[0];
    if (!remote) return null;
    const destination = await gitPrProvider(cwd, remote.name, workstream.branch);
    const checks =
      destination.provider === "github" && destination.pr
        ? await gitPrChecks(cwd).catch(() => [])
        : [];
    return { pr: destination.pr, checks };
  } catch (error) {
...
export async function updateWorkstreamFromBase(
  ...
  try {
    const context = await ciContext(cwd);
...
function ticketLine(link: LinkedWorkItem): string {
  const ref =
    link.identifier ||
    (link.provider === "azure" && link.number
      ? `AB#${link.number}`
      : `#${link.number}`);
...
/**
 * Create a PR per workstream that doesn't have one, then patch GitHub bodies
 * with the sibling URLs (they only exist once creation returns). `only`
 * restricts creation to specific workstreams while keeping the full task
 * context — sibling branches and existing PR URLs — in the body. Azure PRs
 * keep ticket references — Azure auto-links `AB#<id>` mentions.
 */
export async function createTaskPrs(
  ...
  for (const ws of task.workstreams) {
    ...
    try {
      ...
      const remote =
        context.remotes.find((entry) => entry.name === "origin") ??
        context.remotes[0];
      if (!remote) throw new Error("No git remote");
      await gitSync(cwd);
      const destination = await gitPrProvider(cwd, remote.name, ws.branch);
      if (destination.pr) {
        results.push({
          workstreamId: ws.id,
          ok: true,
      ...
      const base = context.remotes.some((entry) =>
        ws.base.startsWith(`${entry.name}/`),
      )
      ...
      const siblings = [...knownUrls().entries()]
        .filter(([id]) => id !== ws.id)
        .map(([, url]) => url);
      const url = await destination.create({
        title: task.title,
        body: prBody(task, ws, siblings),
        base,
        head: ws.branch,
      });
      created.push({ ws, url, provider: destination.provider });
  ...
  // Second pass: GitHub bodies get the real sibling URLs.
