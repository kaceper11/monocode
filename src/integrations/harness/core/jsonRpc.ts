...
export class JsonRpcClient {
  ...
  async request<T>(
    ...
    // The deadline may still settle `response` while the send is in flight;
    // mark it handled so that window can't surface as an unhandled rejection.
  ...
  private async send(payload: object, signal?: AbortSignal): Promise<void> {
    ...
    try {
      await Promise.race([
        writeChild(this.sessionId, line, signal),
