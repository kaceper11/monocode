import { promptBlocks, type PromptContentBlock } from "../../../features/sessions/model/attachments.ts";
import type { Attachment, RuntimeMode, ToolPreview } from "../../../features/sessions/model/session.ts";
import { normalizeTaskListStatus } from "../../../features/sessions/model/taskList.ts";
import type { UserQuestion, UserQuestionReply } from "../../../features/sessions/model/userQuestion.ts";
import {
...
export function acpElicitationResult(
  ...
  for (const field of fields) {
    ...
    const values = selected.flatMap((id) => {
      if (Object.prototype.hasOwnProperty.call(field.values, id)) return [field.values[id]];
...
export function acpUnsupportedControl(error: unknown): boolean {
  ...
  if (code != null) return code === -32601;
  return /method not found|not implemented|unknown method/i.test(error instanceof Error ? error.message : String(error));
}
