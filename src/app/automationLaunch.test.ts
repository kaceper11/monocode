...
function fixture(connectAutomationWorkspace = vi.fn().mockResolvedValue(undefined)) {
  ...
  const dependencies = {
    ...
    automationWorkCwd, connectAutomationWorkspace, newSession, sessionsRef, submitSession, submitWithSettlement, updateAutomationRun, wslLocation,
  ...
  return { launch, automation, sessionsRef, submitSession, updateAutomationRun, connectAutomationWorkspace };
}
...
it("a failed background connection creates no session and dispatches no turn", async () => {
  ...
  expect(f.submitSession).not.toHaveBeenCalled();
