...
Child rows show one branch label; activity stays in tooltips/details. The child's hover/focus **…** button opens a compact menu — **Open**, **Copy path**, **Reveal**, **Hide/Show in project**, **Details & cleanup…**, **All worktrees…** and the reviewed **Remove Git worktree…** — instead of embedding the full panel; **Details** and **Remove** delegate to the shared worktree modal, so no check is bypassed. The repository row's **…** and **+** still open the panel list/create views. Rows also mark the owning task (`· Task <name>`) when a live task child claims the copy. **Oldest first** orders known MonoCode activity; unobserved external worktrees remain **Activity unknown**. The timestamp is the latest saved conversation update or recent project open, not a claim about external editors, commit age or deletion safety. Main and active working copies remain protected.
...
**Select** mode on the same panel adds checkboxes, an **All**/**None** toggle over the filtered rows, a **Stale** quick-select beside the count (sharing the 14-day known-activity predicate with the attention row), and a **Remove…** bar for batch cleanup. Each selected checkout re-runs the fresh `git_worktree_safety` preflight before the confirmation lists it; dirty, process-bound, locked, detached, missing, prunable or main entries split into a per-row "needs individual review" list whose **Review** button hands off to the single-entry flow — straight to the guarded removal confirmation for dirty or process-bound entries, and to the detail view for missing, prunable, locked or detached entries where their recovery guidance lives. Confirming runs `git_worktree_remove` sequentially through the surviving main checkout — never force, never stopping processes — and branches and conversation records stay. The main checkout, missing/prunable entries, the current path and any checkout containing the active context cannot be selected at all. Rows show `in use` when they contain the active context and `detached` when they carry no branch. Failures keep a results screen with per-entry messages instead of closing silently.

The attention queue carries the reminder side: one derived `worktree` row per verified repository family when non-main, non-hidden copies are missing/prunable or have no recorded MonoCode use for 14 days. Unknown activity never counts as stale, and the currently open checkout, locked copies and hidden copies never nudge. The row's signature fingerprints the stale path set, so snoozed/dismissed rows resurface only when that set changes; its **Clean up** action opens the worktree panel in select mode with the stale set pre-checked — missing/prunable registrations land in the review list — where every removal still goes through the checks above. Nothing is deleted automatically and no background polling is added — the row is derived from already-published family inventory and recent-open records.
